...
namespace Day4_1 {
    class Program {
        ...
        static void Main(string[] args) {
            ...
            int guardWithMax = 0;
            int maxSleep = 0;
            foreach (KeyValuePair<int, int> pair in sleepCount) {
                if (maxSleep < pair.Value) {
                    maxSleep = pair.Value;
                    guardWithMax = pair.Key;
                }
            }

            int[] maxMap = sleepMap[guardWithMax];
            int maxMinute = 0;
            for (int i = 0; i < maxMap.Length; i++) {
                if (maxMap[i] > maxMap[maxMinute]) {
                    maxMinute = i;
                }
            }

            writer.Write(maxMinute * guardWithMax);
            writer.Flush();
#if !ONLINE_JUDGE
            writer.Close();
#endif
